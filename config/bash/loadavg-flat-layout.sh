...
gn=$(echo -e ${green})
byl=$(echo -e ${bold}${a_brightyellow2})
yl=$(echo -e ${yellow})
rd=$(echo -e ${bold}${red})
...
# output
if [ "$FORMAT" = "A" ]; then
    echo -e "$tabs ${bdwt}LOAD AVERAGES${reset}" | indent25
    ...
    text="${bdwt}LOAD AVERAGES${reset}"; printf "\n%*s%s\n" $spaces1 '' "$text"
    ...
    text="${bdwt}LOAD AVERAGE${reset}: $(printf "1-minute: $ONE_MIN  |  5-minute: $FIVE_MIN  |  15-minute: $FIF_MIN")"
    printf "%*s%s\n\n" $spaces2 '' "$text"
